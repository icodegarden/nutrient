...
public abstract class DataSourceUtils {
	...
	private static LinkedHashMap<String, DataSource> dataSources(ShardingSphereDataSource dataSource) {
		try {
			Field contextManagerF = dataSource.getClass().getDeclaredField("contextManager");
			boolean b = contextManagerF.canAccess(dataSource);
			contextManagerF.setAccessible(true);
			ContextManager	contextManager =(ContextManager)	contextManagerF.get(dataSource);
			contextManagerF.setAccessible(b);
			contextManager.getMetaDataContexts().getMetaData().getDatabases();
			ShardingSphereDatabase database = contextManager.getMetaDataContexts().getMetaData().getDatabase("logic_db");
			LinkedHashMap<String, DataSource> dataSources = (LinkedHashMap) 	database.getResourceMetaData().getDataSourceMap();
			return dataSources;
		}catch (Exception e) {
...
//		ShardingSphereMetaData shardingSphereMetaData = contextManager.getMetaDataContexts()
