...
import redis.clients.jedis.commands.FunctionCommands;
...
public interface FunctionBinaryCommands {
	 ...
	 * serialized payload later with the
	 * {@link FunctionBinaryCommands#functionRestore(byte[], FunctionRestorePolicy)
	 ...
	 * 'lazyfree-lazy-user-flush' configuration directive sets the effective
	 * behavior.
	 * 
	 * @return OK
	 */
	 ...
	 * behavior.
	 * 
	 * @param mode ASYNC: Asynchronously flush the libraries, SYNC: Synchronously
	 *             flush the libraries.
	 * @return OK
	 */
	...
	/**
	 * Kill a function that is currently executing. The command can be used only on
	 * functions that did not modify the dataset during their execution.
	 * 
	 * @return OK
	 */
	String functionKill();
	...
	/**
	 * Similar to {@link FunctionCommands#functionList() FUNCTION LIST} but include
	 * the libraries source implementation in the reply.
	 * 
	...
	/**
	 * Similar to {@link FunctionBinaryCommands#functionList(byte[]) FUNCTION LIST}
	 * but include the libraries source implementation in the reply.
	 * 
	 ...
	 * 
	 * @param functionCode the source code. The library payload must start with
	 *                     Shebang statement that provides a metadata about the
	 *                     library (like the engine to use and the library name).
	...
	/**
	 * Load a library to Redis. Will replace the current library if it already
	 * exists.
	 * 
	 ...
	 * 
	 * @param serializedValue the serialized payload
	 * @param policy          can be {@link FunctionRestorePolicy FLUSH, APPEND or
	 *                        REPLACE}
	 * @return OK
	 */
	...
	/**
	 * Return information about the function that's currently running and
